...
// tăng “màu” nhẹ cho nền + giảm bóng marker
(function(){
...
// NEW: helpers để quyết định flyTo khi ở quá xa/zoom nhỏ
...
function updateNearestNumbers(){
  ...
  for (let i=0;i<arr.length;i++){
    ...
    it.marker.setIcon(makeNumIconCached(i+1, c));
  }
}
...
function bindUI(){
  document.getElementById('reload')?.addEventListener('click', async ()=>{
    try{ await loadCSV(); }catch(e){ $status.textContent='Lỗi: '+(e.message||e); }
  });
  ...
  document.getElementById('btnMyLocation')?.addEventListener('click', async ()=>{
    ...
    // Gọi GPS để cập nhật chính xác & lưu Supabase
    startMyLocation(ma_nv, /*forceFly*/ true);
  });

  document.getElementById('btnFollow')?.addEventListener('click',()=>{
    FOLLOW_MY_DOT=!FOLLOW_MY_DOT;
    document.getElementById('btnFollow').style.opacity=FOLLOW_MY_DOT?'1':'.55';
  });
